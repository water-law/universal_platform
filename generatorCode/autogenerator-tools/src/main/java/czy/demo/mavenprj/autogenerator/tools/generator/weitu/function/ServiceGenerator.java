 ...
 */
public enum ServiceGenerator implements CodeGenerator {
    INSTANCE;
    ...
    public void create(GenaratorBaseConfigVo baseConfigVo, CodeConfigInfoVo codeConfigInfo, Configuration configuration) throws Exception {
        ...
        if (SString.isBlank(SString.getTrimString(baseConfigVo.getProjectPath()))) {
            ...
            savePath = SPathUtils.getCompleteJavaFilePath(baseConfigVo.getProjectPath(), baseConfigVo.getProjectPackageName(), baseConfigVo.getModule())
                    + GeneratorConstants.CreateFileConstants.SERVICE_FILE_PATH;
        ...
        Writer serviceWriter = new OutputStreamWriter(new FileOutputStream(savePath + "I" + clazzInfo.getClassName()
                            + GeneratorConstants.CreateFileConstants.CREATE_FILE_NAME_SERVICE_SUFFIX + GeneratorConstants.CreateFileConstants.CREATE_FILE_TYPE_JAVA));
        Template serviceTemplate = configuration.getTemplate(GeneratorConstants.FreemarkerConstants.DEFAULT_TEMPLATE_SERVICE_PATH_FILE_NAME + "/"
                + GeneratorConstants.FreemarkerConstants.TEMPLATE_SERVICE_FILE_NAME + GeneratorConstants.FreemarkerConstants.FREEMARKER_TEMPLATE_FILE_TYPE);
        ...
        Writer serviceImplWriter = new OutputStreamWriter(new FileOutputStream(implSavePath + clazzInfo.getClassName()
                            + GeneratorConstants.CreateFileConstants.CREATE_FILE_NAME_SERVICEIMPL_SUFFIX + GeneratorConstants.CreateFileConstants.CREATE_FILE_TYPE_JAVA));
        //获取模板
        Template serviceImplTemplate = configuration.getTemplate(GeneratorConstants.FreemarkerConstants.DEFAULT_TEMPLATE_SERVICE_PATH_FILE_NAME + "/"
                + GeneratorConstants.FreemarkerConstants.TEMPLATE_SERVICE_IMPL_FILE_NAME + GeneratorConstants.FreemarkerConstants.FREEMARKER_TEMPLATE_FILE_TYPE);
        serviceImplTemplate.process(map, serviceImplWriter);
        System.out.println("----- Generator Service SUCCESS -------");
