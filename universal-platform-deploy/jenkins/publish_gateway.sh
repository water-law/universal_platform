...
echo "copy war"
tomcat_home=$HOME/tomcat8.5.57-gateway
...
echo "-------开始复制ROOT.war---------"
#cp -f $HOME/runtime/gateway/mps-platform-gateway-2.0.0.war $tomcat_home/webapps/ROOT.war
mkdir $tomcat_home/webapps/ROOT
cd $tomcat_home/webapps/ROOT
jar xf $HOME/runtime/gateway/mps-platform-gateway-2.0.0.war
/home/crs/bin/replace_host.sh $tomcat_home
echo "-------结束复制ROOT.war---------"

echo "start tomcat"
export BUILD_ID=mps-platform-gateway
